...
| Core (Planner, Marker, Verifier, Gate, Guide, models, store) | ✅ Done — `src/stepspotter/`; `python -m pytest -q` → **114 passed, 1 skipped** with the `agentcore` extra (the skip needs live AWS credentials); **101 passed, 2 skipped** from a plain `.[dev]` install |
...
pip install -e ".[dev]"
python -m pytest -q      # 101 passed, 2 skipped — no AWS account, no credentials, no network
```

That is the whole cold start. The two skips are the tests that need something you
don't have yet: live AWS credentials, and the optional AgentCore SDK.
